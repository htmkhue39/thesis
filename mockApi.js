...
mock.onGet(new RegExp(`${API_URL}/nodes`)).reply(config => {
    ...
    if (address) {
      const filteredNodes = nodes.filter(node => node.address.toLowerCase().includes(address.toLowerCase()));
...
mock.onPost(`${API_URL}/initialLiquidity`).reply(config => {
    ...
    const newPool = {
      ...
      totalLiquidityTokens: (parseFloat(fromAmount) + parseFloat(toAmount)).toString(),
      transactions: 0,
      tvl: (parseFloat(fromAmount) + parseFloat(toAmount)).toString(),
      dayVolume: 0,
      weekVolume: 0,
      apr: fee,
      liquidityProviderPositions: [{ accountId, amountA: fromAmount, amountB: toAmount, liquidityToken: "UNI-V2" }]
    };
  
    node.liquidityPools.push(newPool);
    return [201, { success: true, priceFromTo: (fromAmount / toAmount).toFixed(4), priceToFrom: (toAmount / fromAmount).toFixed(4), poolShare: '100%' }];
});
  
mock.onPost(`${API_URL}/liquidity`).reply(config => {
    const { accountId, nodeAddress, fromToken, toToken, fromAmount, toAmount } = JSON.parse(config.data);
    ...
    pool.totalLiquidityTokens = (parseFloat(pool.totalLiquidityTokens) + parseFloat(fromAmount) + parseFloat(toAmount)).toString();
    pool.tvl = (parseFloat(pool.tvl) + parseFloat(fromAmount) + parseFloat(toAmount)).toString();
  
    let providerPosition = pool.liquidityProviderPositions.find(position => position.accountId === accountId);
    if (providerPosition) {
      providerPosition.amountA = (parseFloat(providerPosition.amountA) + parseFloat(fromAmount)).toString();
      providerPosition.amountB = (parseFloat(providerPosition.amountB) + parseFloat(toAmount)).toString();
    } else {
      pool.liquidityProviderPositions.push({ accountId, amountA: fromAmount, amountB: toAmount, liquidityToken: "UNI-V2" });
    }
  
    const priceFromTo = (pool.totalLiquidityTokenA / pool.totalLiquidityTokenB).toFixed(4);
    const priceToFrom = (pool.totalLiquidityTokenB / pool.totalLiquidityTokenA).toFixed(4);
    const poolShare = ((parseFloat(fromAmount) + parseFloat(toAmount)) / parseFloat(pool.totalLiquidityTokens) * 100).toFixed(2) + '%';
...
mock.onGet(new RegExp(`${API_URL}/orderBook`)).reply(config => {
  ...
  const tradeHistory = token === 'A' ? orderBook.tradeHistoryA : orderBook.tradeHistoryB;
...
mock.onGet(`${API_URL}/searchOrderBooks`).reply(config => {
    ...
    if (searchQuery) {
      ...
      filteredOrderBooks = orderBooks.filter(ob => 
        ob.tokenA.toLowerCase().includes(query) || ob.tokenB.toLowerCase().includes(query)
      );
    }
