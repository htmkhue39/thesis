...
const mnemonics = [
  ["brush", "swing", "cram", "width", "spoon", "visual", "typical", "recipe", "obvious", "damp", "smart", "depend"],
  ["moon", "galaxy", "star", "planet", "comet", "orbit", "solar", "lunar", "space", "alien", "rocket", "satellite"]
];
...
const nodes = [
  {
    ...
    tokens: [
      ...
      { id: "3", name: "Tether", symbol: "USDT", logo: "/coin/cardano.png", price: "$0.998", change1h: "0.03", change24h: "-0.08", change7d: "-0.06", volume24h: "$34.04B" },
      { id: "4", name: "BNB", symbol: "BNB", logo: "/coin/binance.png", price: "$585.21", change1h: "0.68", change24h: "-2.7", change7d: "-3.65", volume24h: "$923.73M" },
      { id: "5", name: "Solana", symbol: "SOL", logo: "/coin/solana.png", price: "$132.16", change1h: "0.85", change24h: "-3.85", change7d: "-10.55", volume24h: "$1.98B" },
      { id: "6", name: "Lido Staked Ether", symbol: "STETH", logo: "/coin/shiba-inu.png", price: "$3.5k", change1h: "1.01", change24h: "-1.99", change7d: "-0.15", volume24h: "$61.87M" },
      { id: "7", name: "USD Coin", symbol: "USDC", logo: "/coin/usdc.png", price: "$0.999", change1h: "-0.06", change24h: "-0.04", change7d: "-0.04", volume24h: "$4.37B" },
      { id: "8", name: "XRP", symbol: "XRP", logo: "/coin/ripple.png", price: "$0.488", change1h: "0.33", change24h: "-1.29", change7d: "2.58", volume24h: "$827.27M" },
      { id: "9", name: "Dogecoin", symbol: "DOGE", logo: "/coin/dogecoin.png", price: "$0.124", change1h: "1.21", change24h: "-0.8", change7d: "-12.61", volume24h: "$615.12M" },
      { id: "10", name: "Toncoin", symbol: "TON", logo: "/coin/usdt.png", price: "$7.12", change1h: "1.13", change24h: "-0.84", change7d: "-10.44", volume24h: "$374.47M" }
    ],
    liquidityPools: [
      {
        id: "1",
        ...
        liquidityProviderPositions: [
          { accountId: "1", amountA: "5", amountB: "2500", liquidityToken: "UNI-V2" }
        ]
      },
      {
        id: "2",
        ...
        apr: "0.000%",
        liquidityProviderPositions: [
          { accountId: "1", amountA: "2", amountB: "5000", liquidityToken: "UNI-V2" }
        ]
      },
      {
        id: "3",
        ...
        apr: "0.003%",
        liquidityProviderPositions: [
          { accountId: "1", amountA: "3", amountB: "3000", liquidityToken: "UNI-V2" }
        ]
      }
    ],
    orderBook: {
...
mock.onPost(`${API_URL}/checkAccount`).reply(config => {
  ...
  const account = accounts.find(acc => JSON.stringify(acc.mnemonic) === JSON.stringify(mnemonic) && acc.passcode === password);
...
mock.onPost(`${API_URL}/transactions`).reply(config => {
    ...
    const accountIndex = accounts.findIndex(account => account.address === newTransaction.address);
  
    if (accountIndex !== -1) {
      accounts[accountIndex].transactions = accounts[accountIndex].transactions || [];
...
mock.onPost(`${API_URL}/checkPool`).reply(config => {
    ...
    if (node) {
        ...
        if (pool) {
            const priceFromTo = (pool.totalLiquidityTokenA / pool.totalLiquidityTokenB).toFixed(4);
            const priceToFrom = (pool.totalLiquidityTokenB / pool.totalLiquidityTokenA).toFixed(4);
            const poolShare = ((pool.totalLiquidityTokenA + pool.totalLiquidityTokenB) / pool.totalLiquidityTokens * 100).toFixed(2) + '%';
...
mock.onPost(`${API_URL}/addLiquidity`).reply(config => {
    const { accountId, nodeAddress, fromToken, toToken, fromAmount, toAmount, fee } = JSON.parse(config.data);
    
    const account = accounts.find(acc => acc.id === accountId);
    if (!account) return [404, { message: 'Account not found' }];
  
    const channel = channels.find(channel =>
      (channel.address_a === account.address && channel.address_b === nodeAddress) ||
      (channel.address_a === nodeAddress && channel.address_b === account.address)
    );
    if (!channel) return [404, { message: 'Channel not found' }];
  
    let pool = channel.liquidityPositions.find(
      pool =>
    ...
    if (!pool) {
        pool = {
            ...
            totalPoolTokens: (parseFloat(fromAmount) + parseFloat(toAmount)).toString()
        };
        channel.liquidityPositions.push(pool);
        poolShare = "100%";
    } else {
        pool.amountA = (parseFloat(pool.amountA) + parseFloat(fromAmount)).toString();
        pool.amountB = (parseFloat(pool.amountB) + parseFloat(toAmount)).toString();
        pool.totalPoolTokens = (parseFloat(pool.totalPoolTokens) + parseFloat(fromAmount) + parseFloat(toAmount)).toString();
        poolShare = ((parseFloat(fromAmount) + parseFloat(toAmount)) / parseFloat(pool.totalPoolTokens) * 100).toFixed(2) + '%';
    }
  
    channel.balances[fromToken] = (parseFloat(channel.balances[fromToken] || 0) - parseFloat(fromAmount)).toString();
    channel.balances[toToken] = (parseFloat(channel.balances[toToken] || 0) - parseFloat(toAmount)).toString();
...
mock.onGet(new RegExp(`${API_URL}/orderBook`)).reply(config => {
    const { nodeAddress } = config.params || {};
    const node = nodes.find(n => n.address === nodeAddress);
    if (node) {
      return [200, node.orderBook];
    }
    return [404, { message: 'Order book not found' }];
});
  
mock.onPost(`${API_URL}/orders`).reply(config => {
    const { nodeAddress, type, price, amount } = JSON.parse(config.data);
    const node = nodes.find(n => n.address === nodeAddress);
    if (!node) {
        return [404, { message: 'Node not found' }];
    }

    const newOrder = { type, price, amount };
    if (type === 'buy') {
        node.orderBook.bids.push(newOrder);
    } else {
        node.orderBook.asks.push(newOrder);
    }
...
export const addLiquidity = async (data) => {
    try {
      const response = await axios.post(`${API_URL}/addLiquidity`, data);
      return response.data;
    } catch (error) {
...
const findOrCreatePool = (nodeAddress, tokenA, tokenB, fee, amountA, amountB) => {
    let node = nodes.find(node => node.address === nodeAddress);
    if (!node) return null;

    let pool = node.liquidityPools.find(pool => 
    ...
    if (!pool) {
        pool = {
            ...
            totalLiquidityTokens: amountA + amountB,
            transactions: 0,
            tvl: amountA + amountB,
            dayVolume: 0,
            weekVolume: 0,
        ...
        node.liquidityPools.push(pool);
    } else {
        pool.totalLiquidityTokenA += amountA;
        pool.totalLiquidityTokenB += amountB;
        pool.totalLiquidityTokens += amountA + amountB;
        pool.tvl += amountA + amountB;
    }

    const priceFromTo = (amountA / amountB).toFixed(4);
    const priceToFrom = (amountB / amountA).toFixed(4);
    const poolShare = ((amountA + amountB) / pool.totalLiquidityTokens * 100).toFixed(2) + '%';

    return { pool, priceFromTo, priceToFrom, poolShare };
};

export const getOrderBook = async (nodeAddress) => {
    try {
      const response = await axios.get(`${API_URL}/orderBook`, { params: { nodeAddress } });
      return response.data;
    } catch (error) {
